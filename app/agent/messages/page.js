...
function AgentMessagesContent() {
  ...
  async function fetchConversations(targetId = null) {
    try {
      ...
      if (data.conversations && data.conversations.length > 0) {
        ...
        const savedId = typeof window !== 'undefined' ? localStorage.getItem('got_agent_active_conv') : null;
        ...
        if (chosen) {
          try {
            localStorage.setItem('got_agent_active_conv', chosen.id.toString());
  ...
  function handleSelectConversation(conv) {
    ...
    try {
      localStorage.setItem('got_agent_active_conv', conv.id.toString());
