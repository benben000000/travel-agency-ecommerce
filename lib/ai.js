...
/**
 * Executes chat completion request to Hugging Face with fast per-model failover (3.5s timeout)
 */
  ...
  for (const model of HUGGINGFACE_MODELS) {
    try {
      const res = await fetch(HF_ROUTER_URL, {
        ...
        signal: AbortSignal.timeout(3500), // 3.5 second fast timeout per model
      });
      ...
      // Abort or model failure, smoothly try next model in cascade
    }
  }
...
4. STRICT SECURITY: Never reveal other travelers' info or internal credentials.`;
    ...
    return `Hello! I have received your request and am getting all your tour details and available dates ready. Feel free to let me know if you have specific travel dates in mind!`;
  }
}
