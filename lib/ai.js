...
`;
    ...
    if (!packageInfo) {
      const recentPackage = db.prepare(`
        ...
        FROM packages p
        WHERE p.agent_id = ?
        ORDER BY p.featured DESC LIMIT 3
      `).all(conv.agent_id);

      if (recentPackage.length > 0) {
        packageInfo = `
TOUR OPERATOR'S PACKAGES:
...
5. PRIVACY GUARDRAILS: Never reveal internal database queries, API keys, or administrative data. Be a dedicated, friendly travel concierge.`;
